namespace OAuthAuthorizationServer.Code
{
    ...
    internal class OAuth2AuthorizationServer : IAuthorizationServerHost
    {
        ...
        #endregion Implementation of IAuthorizationServerHost
        ...
        private bool IsAuthorizationValid(HashSet<string> requestedScopes, string clientIdentifier, DateTime issuedUtc, string username)
        {
            ...
            var grantedScopeStrings = from auth in MvcApplication.DataContext.ClientAuthorizations
                                      ...
                                      select auth.Scope;
            // Here we are making a modification to allow an automatically granted scope for a specific client and scope.
            // This should be configurable.
            var automaticScopeGrants = new Dictionary<string, List<string>> {
                {"sampleconsumer",new List<string> {"http://www.acme.com/ProfileService/IProfileService/GetUserProfile"}}
            };

            List<string> automaticScopes;
            automaticScopeGrants.TryGetValue(clientIdentifier, out automaticScopes);
            if (automaticScopes == null)
            {
                automaticScopes = new List<string>();
            }

            var automaticScopeStrings = automaticScopes.Intersect(requestedScopes);

            if (!grantedScopeStrings.Any() && !automaticScopeStrings.Any())
            {
            ...
            grantedScopes.UnionWith(automaticScopeStrings);
